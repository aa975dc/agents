...
这是 ZCode 插件 `dev-companion`，版本 `0.2.0`。包含七个命令、一项共享技能、两个固定子代理（开发者与独立检查者）加三个按需设计角色（产品、界面、后端），以及使用 Python 3.9+ 标准库的本地工具（`scripts/_kernel_vendor/` 为构建生成的共享内核私有副本）。没有必装 MCP、Hook 或云服务。
