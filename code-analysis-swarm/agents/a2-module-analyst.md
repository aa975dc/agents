...
1. **通读闭集**：逐文件阅读（大文件可抓重点段，但入口与导出必须全读）。
...
6. **写契约**：为块内每个对外暴露的模块写 `interfaces/<模块名>.md`（≤50 行：一句话职责 + 导出签名 + 依赖声明 + 一条典型用法）。
...
modules:
  - { name, responsibility, entry_files: [path],
      ...
      patterns: [观察到的模式] }
edges: [ { from, to, kind: "import|call|config", from_contract: bool } ]
findings:
  - { where: "path:line", what, evidence, severity: "low|medium|high", confidence: 0~1 }
coverage: { files_claimed: int, files_analyzed: int }
```
...
- **证据强制**：findings 每条带 `file:line`；coverage 两数不等必须在返回中说明缺口原因
